...
class CustomManager(models.Manager):
    def get_queryset(self):
        ...
        for row in cursor.fetchall():
            p = self.model(role_id=row[0], right_id=row[1])
            result_list.append(p)
        return result_list

class RoleRights(models.Model): # TODO: No primary key in table
...
class Users(models.Model):
    id = models.IntegerField(primary_key=True)#models.ForeignKey('NodesHierarchy', primary_key=True, db_column='id', limit_choices_to={'node_type_id', 14})
